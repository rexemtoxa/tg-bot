...
export interface BotDeps {
  ...
  command(
    command: string,
    callback: (ctx: Context & { message: Update.New & Update.NonChannel & Message.TextMessage }) => Promise<void>
  ): void;
  launch(): Promise<void>;
  telegram: {
...
export const launchBot = (dbDeps: DataBaseDeps, bot: BotDeps, admins: string[], baseUrl: string): void => {
  ...
  const isCommandMessageValid = (
    ctx: Context & { message: Update.New & Update.NonChannel & Message.TextMessage }
  ): boolean => {
    if (!ctx.message?.text) {
      return false;
    }
